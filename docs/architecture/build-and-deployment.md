...
**`/cms` excluded from prerender** — the Decap CMS admin UI at `public/cms/` is a standalone SPA. The prerender filter skips `/cms` paths so TanStack Start doesn't try to render them as routes.
