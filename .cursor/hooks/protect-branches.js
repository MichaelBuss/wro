...
const GH_API_PROTECTED_BRANCH_PATH_PATTERN =
  /(?:branches\/|ref\/heads\/|refs\/heads\/)(main)(?=$|[/?\s])/
...
const isProtectedBranch = (value) => {
  ...
  return normalizedBranchName
    ? PROTECTED_BRANCHES.has(normalizedBranchName)
    : false
}
...
const isEnvironmentAssignment = (token) =>
...
const unwrapShellSegment = ({ tokens, cwd }) => {
  ...
  while (
    ...
    isEnvironmentAssignment(currentTokens[0])
  ) {
  ...
  if (currentTokens[0] === 'env') {
    ...
    while (index < currentTokens.length) {
      ...
      if (token.startsWith('--chdir=')) {
        effectiveCwd = resolveCommandPath(
          effectiveCwd,
  ...
  if (
    currentTokens[0] === 'command' ||
    currentTokens[0] === 'nohup' ||
    currentTokens[0] === 'time'
  ) {
    return unwrapShellSegment({
  ...
  if (
    currentTokens[0] === 'bash' ||
    currentTokens[0] === 'sh' ||
    currentTokens[0] === 'zsh'
  ) {
    const commandIndex = currentTokens.findIndex(
      (token) => token === '-c' || token === '-lc' || token === '-ic',
    )
...
const getGitExecutionContext = ({ cwd, gitArgs }) => {
  ...
  while (index < gitArgs.length) {
    ...
    if (token.startsWith('--git-dir=')) {
      gitDir = resolveCommandPath(
        currentDirectory,
        token.slice('--git-dir='.length),
      )
      index += 1
      continue
    }
    ...
    if (token.startsWith('--work-tree=')) {
      workTree = resolveCommandPath(
        currentDirectory,
        token.slice('--work-tree='.length),
      )
      index += 1
      continue
    }
  ...
  const repoPath =
    workTree || deriveRepoPathFromGitDir(gitDir) || currentDirectory
...
const findBranchCreatedByGitCommand = (subcommandArgs) => {
  for (let index = 0; index < subcommandArgs.length; index += 1) {
    ...
    if (
      ...
      return (
        normalizeBranchName(subcommandArgs[index + 1]) ||
        subcommandArgs[index + 1] ||
        null
      )
    }
    ...
    if (argument.startsWith('--create=')) {
      return (
        normalizeBranchName(argument.slice('--create='.length)) ||
        argument.slice('--create='.length)
      )
    }
  }
...
const getNextBranchAfterGitCommand = ({
  currentBranch,
  subcommand,
  subcommandArgs,
}) => {
...
const evaluateGitCommand = ({ currentBranch, subcommand, subcommandArgs }) => {
  ...
  if (
    PROTECTED_BRANCH_MUTATING_GIT_SUBCOMMANDS.has(subcommand) &&
...
const evaluateGhCommand = (ghArgs) => {
  ...
  if (ghArgs[0] === 'api') {
    ...
    const protectedBranchPathMatch = apiArguments
      .join(' ')
      .match(GH_API_PROTECTED_BRANCH_PATH_PATTERN)
...
const shouldBlockShellCommand = ({ command, cwd, execFn = execSync }) => {
  ...
  for (const commandSegment of commandSegments) {
    ...
    if (
      unwrappedSegment.type === 'other' &&
      containsGitOrGhInvocation(commandSegment)
    ) {
      return deny({
        userMessage: 'Blocked unclassified git/gh command wrapper.',
        agentMessage:
          'Blocked unclassified git/gh command wrapper because the hook cannot safely inspect it.',
      })
    }
  }
